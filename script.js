...
// 🏷️ Tags-Eingabefeld automatisch ergänzen (falls noch nicht da)
function addTagsInput() {
  const form = document.querySelector("#shortcutForm");
  if (!form || form.querySelector("#tagsInput")) return;
...
function searchShortcuts(term) {
  ...
  const results = allShortcuts.filter(sc => {
    const inName = sc.name?.toLowerCase().includes(term);
    const inTags = (sc.tags || []).some(tag => tag.includes(term));
...
function displaySearchResults(results) {
  ...
  results.forEach(sc => {
    ...
    div.innerHTML = `
      ...
      <small><b>Tags:</b> ${(sc.tags || []).join(", ")}</small>
    `;
